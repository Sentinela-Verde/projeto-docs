...
- persistência de features reutilizáveis e de saídas técnicas de modelos, como classificações, probabilidades, scores, candidatos e atribuições, desde que ainda não incorporem regra de negócio ou visão analítica final.
...
**Regra:** Cada transformação precisa ser registrada e deve respeitar o contrato versionado. Saídas técnicas de modelos pertencem à Silver quando representam resultados reutilizáveis do processamento/modelagem; os artefatos binários do modelo (`.joblib`, `.pkl`, checkpoints e equivalentes) permanecem fora do Lakehouse, no bucket de artifacts.
---
...
Cada tabela Gold representa um analytical product e deve declarar caso de uso, grain, `business_key`, measures, dimensions, consumidores e freshness. Gold contém exclusivamente datasets analíticos governados orientados a consumo, regras de negócio, indicadores, métricas finais e visões consolidadas. Saídas técnicas de modelos não devem ser promovidas à Gold apenas por serem resultados de ML; elas permanecem na Silver e alimentam os produtos analíticos Gold. Gráficos renderizados, relatórios, imagens e demais artifacts de apresentação permanecem fora da camada Gold e devem manter lineage até o dataset governado correspondente.
Metadados técnicos obrigatórios: `created_at`, `updated_at`, `execution_id`.
**Princípio:** Gold representa a camada de visão analítica e regra de negócio. Cada produto Gold deve possuir rastreabilidade até as tabelas Silver/Bronze e, quando aplicável, até a versão do modelo técnico que produziu os insumos em Silver.
...
- [Governança — Histórico de Revisões](historico-revisoes.md)
